(function(){
  ...
  const BUILD='879-r9.4-manage-export-xls';
  ...
  function ensureStyle(){
    if(el('waterExportR94Style'))return;
    const s=document.createElement('style');
    s.id='waterExportR94Style';
    s.textContent=`
      #waterExportR94 .r94Label{display:block;font-size:11px;font-weight:800;color:#5c6975;margin:3px 0 6px;text-transform:uppercase}
      #waterExportR94 .r94Row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:end}
      #waterExportR94 select{width:100%;height:40px;border:1px solid #bdc7d1;border-radius:9px;background:#fff;color:#18232d;padding:0 10px;font:700 13px Arial,sans-serif;outline:none}
      #waterExportR94 button{height:40px;border:0;border-radius:9px;background:#174a7e;color:#fff;padding:0 14px;font:800 12px Arial,sans-serif;white-space:nowrap;box-shadow:none}
      #waterExportR94 button:disabled{opacity:.5}
      #waterExportR94Status{margin-top:8px;font-size:11px;line-height:1.4;color:#6a7783;min-height:15px}
      #waterExportR94Status.ok{color:#2a6942}
      #waterExportR94Status.err{color:#a23a2a}
      @media(max-width:360px){#waterExportR94 .r94Row{grid-template-columns:1fr}#waterExportR94 button{width:100%}}
    `;
  ...
  function mount(){
    if(mounted&&el('waterExportR94'))return true;
    const card=findTargetCard();if(!card)return false;
    ensureStyle();
    card.id='waterExportR94';
    card.innerHTML=`
      <div class="waterCardTitle">TẢI FILE CHỈ SỐ</div>
      <label class="r94Label" for="waterExportMonth">Chọn tháng</label>
      <div class="r94Row">
      ...
      <div id="waterExportR94Status">Chỉ hiển thị các tháng đã có chỉ số trên hệ thống.</div>`;
    mounted=true;
  ...
  function status(message,kind){const n=el('waterExportR94Status');if(!n)return;n.className=kind||'';n.textContent=message||'';}
  ...
  async function downloadSelected(){
    ...
    try{
      ...
      const aoa=[headers];
      rows.forEach(function(r){const out=[];for(let i=0;i<headers.length;i++)out.push(excelValue(r&&r.c?r.c[i]:null));aoa.push(out);});
