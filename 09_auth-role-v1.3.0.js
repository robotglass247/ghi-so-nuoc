...
VERSION: 1.3.0-DEV
STATUS: DEV ONLY - POST TOKEN + JSONP RESULT POLL
ROLE MATRIX:
  QUẢN LÝ   -> FILE HỆ THỐNG + TẢI FILE + XEM CHỈ SỐ
  NHÂN VIÊN -> TẢI FILE + XEM CHỈ SỐ
  OTHER     -> DENY ALL
TRANSPORT:
  - Google ID token is POSTed in a hidden form (never placed in URL).
  - Gateway stores a short-lived result keyed by random nonce.
  - Browser retrieves only that result through a short JSONP GET.
============================================================ */
(function(){
  'use strict';
  ...
  if(!core||!ui) throw new Error('AUTH v1.3.0 thiếu dependency');
  ...
  function loadGsi(){return new Promise(function(resolve,reject){if(window.google&&window.google.accounts&&window.google.accounts.id){resolve();return;}const existing=document.querySelector('script[data-water-gsi="1"]');if(existing){existing.addEventListener('load',resolve,{once:true});existing.addEventListener('error',function(){reject(new Error('Không tải được Google Identity Services'));},{once:true});return;}const s=document.createElement('script');s.src='https://accounts.google.com/gsi/client';s.async=true;s.defer=true;s.dataset.waterGsi='1';s.onload=resolve;s.onerror=function(){reject(new Error('Không tải được Google Identity Services'));};document.head.appendChild(s);});}

  function randomNonce(){
    try{
      const a=new Uint32Array(6);crypto.getRandomValues(a);return Array.from(a).map(function(n){return n.toString(36);}).join('-')+'-'+Date.now().toString(36);
    }catch(e){return 'n-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2)+'-'+Math.random().toString(36).slice(2);}
  }

  function ensurePostFrame(){
    let f=document.getElementById('waterAuthPostFrame');
    if(f)return f;
    f=document.createElement('iframe');
    f.id='waterAuthPostFrame';f.name='waterAuthPostFrame';f.setAttribute('aria-hidden','true');
    f.style.cssText='position:absolute!important;width:1px!important;height:1px!important;border:0!important;opacity:0!important;pointer-events:none!important;left:-9999px!important;top:-9999px!important;display:block!important';
    document.body.appendChild(f);return f;
  }

  function submitCredential(credential,nonce){
    const frame=ensurePostFrame();
    const form=document.createElement('form');form.method='POST';form.action=cfg.AUTH_GATEWAY_URL;form.target=frame.name;form.style.display='none';
    function add(name,value){const i=document.createElement('input');i.type='hidden';i.name=name;i.value=String(value==null?'':value);form.appendChild(i);}
    add('credential',credential);add('nonce',nonce);add('app','ghi-so-nuoc-auth-v13');
    document.body.appendChild(form);
    try{form.submit();}catch(e){if(form.parentNode)form.parentNode.removeChild(form);throw e;}
    setTimeout(function(){try{if(form.parentNode)form.parentNode.removeChild(form);}catch(e){}},1500);
  }

  function getResultJsonp(nonce){
    return new Promise(function(resolve,reject){
      const cb='__waterAuthResult_'+Date.now()+'_'+Math.random().toString(36).slice(2);
      const s=document.createElement('script');let done=false;
      const timer=setTimeout(function(){finish(new Error('Gateway không trả kết quả.'));},7000);
      ...
      s.onerror=function(){finish(new Error('Không đọc được kết quả AUTH Gateway.'));};
      s.src=cfg.AUTH_GATEWAY_URL+'?mode=result&nonce='+encodeURIComponent(nonce)+'&callback='+encodeURIComponent(cb)+'&_='+Date.now();
      document.head.appendChild(s);
    });
  }

  async function verifyCredential(credential){
    if(!cfg.AUTH_GATEWAY_URL)throw new Error('DEV chưa cấu hình AUTH_GATEWAY_URL');
    if(!credential)throw new Error('Google không trả ID token.');
    const nonce=randomNonce();
    submitCredential(String(credential),nonce);
    ...
    while(Date.now()<deadline){
      ...
      try{
        const data=await getResultJsonp(nonce);
        if(data&&data.pending===true)continue;
        if(!data||data.ok!==true)throw new Error((data&&data.error)||'Xác thực thất bại');
        return data;
      }catch(err){
        const msg=String(err&&err.message||err);
        if(msg==='Xác thực thất bại'||msg.indexOf('Google ')===0||msg.indexOf('Email ')===0||msg.indexOf('Tài khoản ')===0)throw err;
        readErrors++;
        if(readErrors>=3)throw err;
      }
    }
    throw new Error('Hết thời gian xác minh tài khoản.');
  }

  async function onGoogleCredential(resp){
    try{
      setMsg('Đang xác minh tài khoản và phân quyền...',false);
      const data=await verifyCredential(resp&&resp.credential);
      accept(data);
      if(state.authorized){hideOverlay();ui.set('Đã xác thực: '+(state.name||state.email)+' · '+state.role,'ok');}
      else{setMsg(state.reason||'Tài khoản không được phép sử dụng ứng dụng.',true);ui.set(state.reason||'Tài khoản không được phép sử dụng ứng dụng.','err');}
    }catch(err){const m=String(err&&err.message||err);deny(m);setMsg(m,true);ui.set(m,'err');}
  }

  async function init(){
    if(initialized)return;initialized=true;ensureStyle();deny('Chưa xác thực tài khoản Google');showOverlay();
    if(!cfg.GOOGLE_CLIENT_ID){setMsg('DEV chưa cấu hình GOOGLE_CLIENT_ID.',true);return;}
    try{
      await loadGsi();
      window.google.accounts.id.initialize({client_id:cfg.GOOGLE_CLIENT_ID,callback:onGoogleCredential,auto_select:false,cancel_on_tap_outside:false});
      const host=document.getElementById('waterAuthGoogleButton');if(!host)throw new Error('Không tạo được vùng đăng nhập Google.');host.innerHTML='';
      window.google.accounts.id.renderButton(host,{type:'standard',theme:'outline',size:'large',text:'signin_with',shape:'rectangular',locale:'vi',width:300});
      setMsg('Đăng nhập bằng tài khoản Google đã khai báo trong NHAN_SU_THUC_HIEN.',false);
    }catch(err){const m=String(err&&err.message||err);deny(m);setMsg(m,true);}
  }

  document.addEventListener('WATER_MANAGE_CARD_READY',applyVisual);
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
  window.WATER_AUTH_ROLE={BUILD:'auth-role-v1.3.0-dev-post-cache-jsonp',getState:function(){return JSON.parse(JSON.stringify(state));},rolePermissions:rolePermissions,verifyCredential:verifyCredential,refreshVisual:applyVisual};
})();
