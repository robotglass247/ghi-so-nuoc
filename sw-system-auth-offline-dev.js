/* OFFLINE DEV service worker — FILE HỆ THỐNG auth only */
const CACHE='water-offline-systemauth-dev-v1';
const APP_SHELL=[
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    for(const url of APP_SHELL){
      try{await c.add(new Request(url,{cache:'reload'}));}catch(e){}
    }
  ...
  if(req.mode==='navigate'){
    event.respondWith((async()=>{
      try{
        ...
        const c=await caches.open(CACHE);
        c.put('./r9-system-auth-offline-dev.html',fresh.clone()).catch(()=>{});
        return fresh;
      }catch(e){
        return (await caches.match('./r9-system-auth-offline-dev.html')) || Response.error();
      }
    })());
    return;
  }
