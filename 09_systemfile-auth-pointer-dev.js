/* ============================================================
MODULE_ID: 09-SYSTEMFILE-AUTH-POINTER
STATUS: DEV ONLY
PURPOSE:
  FILE HỆ THỐNG only.
  Reuse the PASS AUTH data model/rules without touching PASS blobs.
  ...
  Does not control TẢI FILE / XEM CHỈ SỐ / capture.
============================================================ */
(function(){
'use strict';
if(window.WATER_SYSTEM_AUTH_POINTER_DEV)return;

const core=window.WATER_MANAGE_CORE;
const ui=window.WATER_MANAGE_STATUS;
const cfg=window.WATER_AUTH_SHEETS_CONFIG||{};
if(!core||!ui)throw new Error('SYSTEM AUTH POINTER thiếu dependency');

const SYSTEM_ID='waterSystemFileR118';
const OVERLAY_ID='waterSystemPointerOverlay';
const MSG_ID='waterSystemPointerMsg';
const LOGIN_ID='waterSystemPointerLogin';
const RESET_MS=3000;
...
function ensureStyle(){
  if(document.getElementById('waterSystemPointerStyle'))return;
  const s=document.createElement('style');
  s.id='waterSystemPointerStyle';
  s.textContent=
...
function activateManage(){
  try{
    const tab=document.getElementById('waterTabManage');
    if(tab)tab.click();
  }catch(e){}
  setTimeout(function(){
    try{
...
function loadGIS(){
  return new Promise(function(resolve,reject){
    ...
    let s=document.querySelector('script[data-water-system-pointer-gis="1"]');
    if(s){
    ...
    s.async=true;s.defer=true;s.dataset.waterSystemPointerGis='1';
    s.onload=resolve;
...
async function verify(token){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    return {
      manager:active&&role==='quan ly',
      employee:active&&role==='nhan vien'
    };
  }
...
function interceptSystem(ev){
  ...
  if(managerVerified){openSystem();return false;}
  showOverlay();
  setMsg('Chọn đúng tài khoản Google đã được cấp quyền QUẢN LÝ.',false);
  return false;
}
...
window.WATER_SYSTEM_AUTH_POINTER_DEV=Object.freeze({BUILD:'systemfile-auth-pointer-dev-v1.0.0'});
})();
