const CACHE='water-v879-final4-guide';
...
const GUIDE=new URL('water-shot-guide-final4.js',self.location.href).href;
...
function patchPageHtml(text){
  ...
  html=html.replace(/<script[^>]+src=["']\.\/water-shot-guide(?:-final3|-final4)?\.js[^"']*["'][^>]*><\/script>\s*/g,'');
  ...
  html=html.replace('</body>',
    ...
    '<script src="./water-shot-guide-final4.js?build='+BUILD+'"></script>\n'+
    '</body>');
  return html;
}
...
async function installCore(){
  ...
  if(!patched.includes('progressBar')||!patched.includes('water-final-core-pre.js')||!patched.includes('water-ui3.js')||!patched.includes('water-final-core-post.js')||!patched.includes('water-shot-guide-final4.js')){
    throw new Error('Trang FINAL4 chưa tạo đúng');
  }

  const [app,pre,ui,post,guide]=await Promise.all([
...
self.addEventListener('message',event=>{
  ...
  event.waitUntil((async()=>{
    const cache=await caches.open(CACHE);
    const [page,app,pre,ui,post,guide]=await Promise.all([
      cache.match(PAGE),cache.match(APP),cache.match(PRE),cache.match(UI),cache.match(POST),cache.match(GUIDE)
    ]);
    const qr=await cache.match(QR);
    event.ports[0].postMessage({
      ready:!!page&&!!app&&!!pre&&!!ui&&!!post&&!!guide,
      build:BUILD,
      qrCached:!!qr
    });
  })());
...
self.addEventListener('fetch',event=>{
  ...
  if(!isApp&&!isPage&&!isPre&&!isUI&&!isPost&&!isGuide&&!isQR)return;
  ...
  event.respondWith((async()=>{
    ...
    const key=isPre?PRE:isUI?UI:isPost?POST:isGuide?GUIDE:isQR?QR:null;
    if(key){
