const CACHE='water-v879-final10-postcapture-r3';
...
const GUIDE=new URL('water-shot-guide-final10-postcapture-r3.js',self.location.href).href;
const CAM=new URL('water-camera-fast-final6.js',self.location.href).href;
const PRE=new URL('water-final-core-pre.js',self.location.href).href;
const POST=new URL('water-final-core-post.js',self.location.href).href;
const QR='https://cdn.jsdelivr.net/npm/jsqr@1.4.0/dist/jsQR.js';
const BUILD='879-final10-postcapture-r3';
...
function patchPageHtml(text){
  ...
  html=html.replace(/<script[^>]+src=["']\.\/water-shot-guide(?:-final3|-final4|-final6|-final9-confirm|-final10-postcapture)?\.js[^"']*["'][^>]*><\/script>\s*/g,'');
  html=html.replace(/<script[^>]+src=["']\.\/water-camera-fast-final6\.js[^"']*["'][^>]*><\/script>\s*/g,'');
  html=html.replace(/<script[^>]+src=["']\.\/water-final-core-(?:pre|post)\.js[^"']*["'][^>]*><\/script>\s*/g,'');
  ...
  html=html.replace(/\s*<div[^>]*>\s*V8\.7\.8\s*·[^<]*<\/div>/i,'');
  ...
  html=html.replace('</body>',
    '<script src="./water-final-core-pre.js?build='+BUILD+'"></script>\n'+
    '<script src="./water-ui3.js?build='+BUILD+'"></script>\n'+
    '<script src="./water-final-core-post.js?build='+BUILD+'"></script>\n'+
    '<script src="./water-camera-fast-final6.js?build='+BUILD+'"></script>\n'+
    '<script src="./water-shot-guide-final10-postcapture-r3.js?build='+BUILD+'"></script>\n'+
    '</body>');
  return html;
}
...
async function installCore(){
  ...
  if(!patched.includes('progressBar')||!patched.includes('water-final-core-pre.js')||!patched.includes('water-ui3.js')||!patched.includes('water-final-core-post.js')||!patched.includes('water-camera-fast-final6.js')||!patched.includes('water-shot-guide-final10-postcapture-r3.js')){
    throw new Error('Trang FINAL10 R3 chưa tạo đúng');
  }

  const [app,pre,ui,post,cam,guide]=await Promise.all([
    fetchFresh(APP+'?release='+BUILD),
    fetchFresh(PRE+'?build='+BUILD),
    fetchFresh(UI+'?build='+BUILD),
    fetchFresh(POST+'?build='+BUILD),
    fetchFresh(CAM+'?build='+BUILD),
  ...
  await Promise.all([
    ...
    cache.put(PRE,pre.clone()),
    cache.put(UI,ui.clone()),
    cache.put(POST,post.clone()),
    cache.put(CAM,cam.clone()),
...
self.addEventListener('message',event=>{
  ...
  event.waitUntil((async()=>{
    const cache=await caches.open(CACHE);
    const [page,app,pre,ui,post,cam,guide]=await Promise.all([
      cache.match(PAGE),cache.match(APP),cache.match(PRE),cache.match(UI),cache.match(POST),cache.match(CAM),cache.match(GUIDE)
    ]);
    const qr=await cache.match(QR);
    event.ports[0].postMessage({
      ready:!!page&&!!app&&!!pre&&!!ui&&!!post&&!!cam&&!!guide,
      build:BUILD,
      qrCached:!!qr
    });
  })());
...
self.addEventListener('fetch',event=>{
  ...
  const prePath=new URL(PRE).pathname;
  const uiPath=new URL(UI).pathname;
  const postPath=new URL(POST).pathname;
  const camPath=new URL(CAM).pathname;
  ...
  const isPre=url.origin===self.location.origin&&url.pathname===prePath;
  const isUI=url.origin===self.location.origin&&url.pathname===uiPath;
  const isPost=url.origin===self.location.origin&&url.pathname===postPath;
  const isCam=url.origin===self.location.origin&&url.pathname===camPath;
  const isGuide=url.origin===self.location.origin&&url.pathname===guidePath;
  const isQR=req.url===QR;
  if(!isApp&&!isPage&&!isPre&&!isUI&&!isPost&&!isCam&&!isGuide&&!isQR)return;
  ...
  event.respondWith((async()=>{
    ...
    const key=isPre?PRE:isUI?UI:isPost?POST:isCam?CAM:isGuide?GUIDE:isQR?QR:null;
    if(key){
