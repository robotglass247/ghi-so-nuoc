(function(){
  ...
  const BUILD='879-r11.12-month-list-from-ghi-so';
  ...
  const MONTH_LIST_RANGE='A1:B40000';
  ...
  function mount(){
    ...
    card.innerHTML=`
      ...
      <div id="waterExportR119Status">
        R11.12 • Đang đọc dữ liệu hệ thống...
      </div>
    `;
    ...
    setTimeout(keepMonthSelectorVisible,1000);
  ...
  async function loadMonths(){
    ...
    try{
      /*
        R11.12
        - Danh sách kỳ: GHI_SO_HANG_THANG cột B.
        - File tải: vẫn bắt buộc từ TAI_CHI_SO_THANG.
      */
      const data=await jsonpSheet(
        MONTH_LIST_SHEET,
        MONTH_LIST_RANGE,
      ...
      // Fallback kỹ thuật nếu danh sách từ GHI_SO_HANG_THANG rỗng.
      if(!list.length){
      ...
      status(
        list.length
          ? 'R11.12 • Chọn tháng: GHI_SO_HANG_THANG • Tải file: TAI_CHI_SO_THANG'
          : 'R11.12 • Chưa tìm thấy kỳ ghi số.',
        list.length?'ok':'err'
      );
      ...
      status(
        list.length
          ? 'R11.12 • Đang dùng danh sách tháng đã lưu. File tải vẫn từ TAI_CHI_SO_THANG.'
          : 'R11.12 • Không đọc được danh sách tháng.',
        'err'
      );
    ...
    try{
      ...
      */
      ...
      if(verifyRange.e.c!==7){
        throw new Error('Khóa R11.12: file tải không đúng 8 cột A:H.');
      }

      for(let rr=0;rr<=verifyRange.e.r;rr++){
        if(verifySheet['I'+(rr+1)]){
          throw new Error('Khóa R11.12: phát hiện cột I/Ảnh trong file tải.');
        }
      }
      ...
      status(
        'R11.12 • '+fileName+' • 8 cột A:H • Không ảnh.',
        'ok'
      );
