/* OFFLINE DEV — exact V2 strategy + lazy unchanged PASS AUTH */
const CACHE='water-offline-authlazy-pass-dev-v1';
const APP_SHELL=[
  './r9-offline-auth-lazy-pass-dev.html',
  './v87-background.html?release=878-fast6&authlazypass=1',
  ...
  './06_view-sheet-v1.js',
  './09_auth-lazy-systemfile-trigger-pass-dev.js',
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('water-offline-authlazy-pass-dev-')&&k!==CACHE).map(k=>caches.delete(k)));
  ...
  if(req.mode==='navigate'){
    event.respondWith((async()=>{
      try{
        ...
        const c=await caches.open(CACHE);
        c.put(new Request('./r9-offline-auth-lazy-pass-dev.html'),fresh.clone()).catch(()=>{});
        return fresh;
      }catch(e){
        return (await caches.match('./r9-offline-auth-lazy-pass-dev.html',{ignoreSearch:true})) || Response.error();
      }
    })());
    return;
  }
