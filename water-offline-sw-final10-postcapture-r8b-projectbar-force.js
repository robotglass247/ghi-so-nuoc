...
const PROJECT=new URL('water-project-bar-r8b.js',self.location.href).href;
...
function injectAddons(html){
  ...
  if(!out.includes('water-shot-style-r7.js')){
    out=out.replace(
      '</body>',
      '<script src="./water-shot-style-r7.js?build='+BUILD+'"></script>\n</body>'
    );
  }

  if(!out.includes('water-project-bar-r8b.js')){
    out=out.replace(
      '</body>',
      '<script src="./water-project-bar-r8b.js?build='+BUILD+'"></script>\n</body>'
    );
  }
...
self.addEventListener('fetch',event=>{
  ...
  const isStyle=url.origin===self.location.origin&&url.pathname===STYLE_PATH;
  const isProject=url.origin===self.location.origin&&url.pathname===PROJECT_PATH;

  if(!isApp&&!isName&&!isLive&&!isStyle&&!isProject)return;
