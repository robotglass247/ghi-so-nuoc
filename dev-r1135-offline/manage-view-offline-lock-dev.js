...
(function(){
  ...
  const BUILD='r1135-month-only-ui-preserve-v15';
  ...
  function readSelect(select){
    ...
    return Array.from(select.options||[]).map(function(o){
      return canon(o.value)||canon(txt(o.textContent).replace(/^Chọn\s*Tháng\s*:\s*/i,''));
    }).filter(Boolean);
  }
  ...
  function loadFinalBehavior(src,key){
    ...
    if(document.querySelector('script[data-r1135-final="'+key+'"]'))return;
    const s=document.createElement('script');
  ...
  loadFinalBehavior('./dev-r1135-unified/manage-download-v7-exact-view-values.js?v=15','WATER_MANAGE_DOWNLOAD_BUILD');
