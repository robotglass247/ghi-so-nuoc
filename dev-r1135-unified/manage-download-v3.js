/* DEV ONLY - TAI FILE theo dung thang dang chon, cung nguon TAI_CHI_SO_THANG.
   V3 giu tab bao tai file + QUAY LAI UNG DUNG.
   File xuat chi co A:H. Cac cot tu I tro di duoc an de file chi hien 8 cot du lieu. */
(function(){
  'use strict';

  const BUILD='r1135-download-v3-hide-after-h';
  ...
  function jsonp(query){
    return new Promise(function(resolve,reject){
      const cb='__waterDownloadV3_'+Date.now()+'_'+Math.random().toString(36).slice(2);
  ...
  async function createAndDownload(period,win){
    ...
    // Excel luon co cac cot sau H. De giao dien file chi con A:H, an toan bo I:XFD.
    for(let c=8;c<16384;c++)cols[c]={hidden:true};
    ws['!cols']=cols;
