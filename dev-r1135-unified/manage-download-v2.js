/* DEV ONLY - TAI FILE theo dung thang dang chon, cung nguon TAI_CHI_SO_THANG.
   V2 khoi phuc tab bao tai file + QUAY LAI UNG DUNG.
   File xuat CHI co 8 cot A:H; bo hoan toan cot I, J, K khoi file tai. */
(function(){
  'use strict';

  const BUILD='r1135-download-v2';
  ...
  function jsonp(query){
    return new Promise(function(resolve,reject){
      const cb='__waterDownloadV2_'+Date.now()+'_'+Math.random().toString(36).slice(2);
  ...
  async function loadPeriod(period){
    ...
    // B:H la 7 cot du lieu can xuat; J chi dung de kiem tra ky, KHONG dua vao file.
  ...
  async function createAndDownload(period,win){
    ...
    // File cuoi cung CHI co A:H = 8 cot. Cot I (anh), J (ky), K (neu co) khong duoc xuat.
    const aoa=[
  ...
  // WINDOW capture de chan handler TAI FILE cu, giu dung 1 lan tai va hien tab trang thai.
