/* DEV ONLY - TAI FILE dung CHINH XAC cung truy van voi XEM CHI SO.
   V6: XEM va TAI cung doc B:J tu TAI_CHI_SO_THANG, cung loc cot J theo ky.
   Xuat Excel chi lay B:H (khong anh, khong ky), giu giao dien V8. */
(function(){
  'use strict';

  const BUILD='r1135-download-v6-same-as-view';
  ...
  /* Giong het XEM CHI SO V3: doc B:I + J, sau do loc lai J o vi tri 8. */
  function jsonp(query){
    return new Promise(function(resolve,reject){
      const cb='__waterDownloadV6_'+Date.now()+'_'+Math.random().toString(36).slice(2);
  ...
  async function loadPeriod(period){
    ...
    /* EXACTLY SAME QUERY AS manage-view-month-v3.js */
  ...
  function renderDone(win,period){
    if(!win)return;
    try{
  ...
  async function createAndDownload(period,win){
    ...
    rows.forEach(function(r,i){
      /* row cua XEM: B,C,D,E,F,G,H,I,J.
         Excel chi lay B:H = indexes 0..6; bo index 7 (Anh), index 8 (Ky). */
      const vals=[];
