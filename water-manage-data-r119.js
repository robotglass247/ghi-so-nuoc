(function(){
  ...
  const BUILD='879-r11.9-system-view-role-download-8cols';
  ...
  function ensureStyle(){
    ...
    s.textContent=`
      ...
      #waterExportR119 .r119Action.disabled{
        opacity:.5;
        pointer-events:none;
      }
  ...
  function mount(){
    ...
    card.innerHTML=`
      ...
      <div class="r119TopRow">
        <div class="r119SelectWrap">
          <select id="waterExportMonthR119" aria-label="Chọn tháng dữ liệu">
      ...
      <div id="waterExportR119Status">
        R11.9 • Đang đọc dữ liệu hệ thống...
      </div>
    `;
    ...
    download.addEventListener('click',function(ev){
      ...
      if(download.disabled || download.classList.contains('disabled'))return false;
      downloadSelected();
      return false;
    },false);
  ...
  async function loadMonths(){
    ...
    try{
      ...
      */
      ...
      status(
        list.length
          ? 'R11.9 • Nguồn: TAI_CHI_SO_THANG • '+list.length+' tháng dữ liệu'
          : 'R11.9 • TAI_CHI_SO_THANG chưa có dữ liệu.',
        list.length?'ok':'err'
      );
    ...
    try{
      ...
      */
      ...
      if(verifyRange.e.c!==7){
        throw new Error('Khóa R11.9: file tải không đúng 8 cột A:H.');
      }

      for(let rr=0;rr<=verifyRange.e.r;rr++){
        if(verifySheet['I'+(rr+1)]){
          throw new Error('Khóa R11.9: phát hiện cột I/Ảnh trong file tải.');
        }
      }
      ...
      status(
        'R11.9 • '+fileName+' • 8 cột A:H • Không ảnh.',
        'ok'
      );
