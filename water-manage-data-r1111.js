(function(){
  ...
  const BUILD='879-r11.11-month-select-always-visible';
  ...
  function mount(){
    ...
    card.innerHTML=`
      ...
      <div id="waterExportR119Status">
        R11.11 • Đang đọc dữ liệu hệ thống...
      </div>
    `;
  ...
  async function loadMonths(){
    ...
    try{
      ...
      */
      ...
      status(
        list.length
          ? 'R11.11 • Nguồn: TAI_CHI_SO_THANG • '+list.length+' tháng dữ liệu'
          : 'R11.11 • TAI_CHI_SO_THANG chưa có dữ liệu.',
        list.length?'ok':'err'
      );
    ...
    try{
      ...
      */
      ...
      if(verifyRange.e.c!==7){
        throw new Error('Khóa R11.11: file tải không đúng 8 cột A:H.');
      }

      for(let rr=0;rr<=verifyRange.e.r;rr++){
        if(verifySheet['I'+(rr+1)]){
          throw new Error('Khóa R11.11: phát hiện cột I/Ảnh trong file tải.');
        }
      }
      ...
      status(
        'R11.11 • '+fileName+' • 8 cột A:H • Không ảnh.',
        'ok'
      );
