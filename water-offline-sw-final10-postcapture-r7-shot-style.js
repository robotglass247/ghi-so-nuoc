...
self.addEventListener('fetch',event=>{
  ...
  if(!isApp&&!isName&&!isLive&&!isStyle)return;
  ...
  event.respondWith((async()=>{
    ...
    if(isName||isLive||isStyle){
      const key=isName?NAME:isLive?LIVE:STYLE;
...
self.addEventListener('message',event=>{
  if(event.data!=='WATER_R7_STYLE_STATUS'||!event.ports[0])return;

  event.waitUntil((async()=>{
    const cache=await caches.open(CACHE);
    const [page,name,live,style]=await Promise.all([
      cache.match(PAGE_R4),
      cache.match(NAME),
      cache.match(LIVE),
      cache.match(STYLE)
    ...
    event.ports[0].postMessage({
      ready:!!page&&!!name&&!!live&&!!style,
      build:BUILD
    });
  })());
});

importScripts('./water-offline-sw-final10-postcapture-r6-liveprogress.js?v='+BUILD);
