const CACHE='water-v876-offline-2';
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    const page=await fetch(new Request(PAGE+'?release=876-time2',{cache:'reload'}));
    if(!page.ok||!(await page.clone().text()).includes('<meta name="water-build" content="876-time2">'))throw new Error('V875 page not published yet');
