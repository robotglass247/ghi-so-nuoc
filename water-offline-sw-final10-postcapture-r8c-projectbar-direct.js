...
const PROJECT=new URL('water-project-bar-r8c.js',self.location.href).href;

const PAGE_PATH=new URL(PAGE_R4).pathname;
...
const PROJECT_PATH=new URL(PROJECT).pathname;
...
function injectAddons(html){
  ...
  if(!out.includes('water-project-bar-r8c.js')){
    out=out.replace('</body>','<script src="./water-project-bar-r8c.js?build='+BUILD+'"></script>\n</body>');
  }
...
self.addEventListener('fetch',event=>{
  ...
  const isPageNav=req.mode==='navigate'&&sameOrigin&&url.pathname===PAGE_PATH;
  const isAppNav=req.mode==='navigate'&&sameOrigin&&url.pathname===APP_PATH;
  ...
  const isProject=sameOrigin&&url.pathname===PROJECT_PATH;

  if(!isPageNav&&!isAppNav&&!isName&&!isLive&&!isStyle&&!isProject)return;
  ...
  event.respondWith((async()=>{
    ...
    if(isName||isLive||isStyle||isProject){
      const key=isName?NAME:isLive?LIVE:isStyle?STYLE:PROJECT;
    ...
    try{
      const fresh=await fetch(PAGE_R4,{cache:'reload'});
...
self.addEventListener('message',event=>{
  ...
  if(event.data!=='WATER_R8C_STATUS'||!event.ports[0])return;

  event.waitUntil((async()=>{
    const cache=await caches.open(CACHE);
    const [page,name,live,style,project]=await Promise.all([
      cache.match(PAGE_R4),
      cache.match(NAME),
      cache.match(LIVE),
      cache.match(STYLE),
    ...
    event.ports[0].postMessage({
      ready:!!page&&!!name&&!!live&&!!style&&!!project,
      build:BUILD
    });
  })());
});
