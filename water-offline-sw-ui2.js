const CACHE='water-v878-offline-ui2';
...
const UI=new URL('water-ui2.js',self.location.href).href;
const BUILD='878-ui2';
...
function patchPageHtml(text){
  ...
  html=html.replace(
    ...
    "const STAFF_CACHE_KEY='water_staff_list_v8';"
  );

  html=html.split('./water-offline-sw.js').join('./water-offline-sw-ui2.js');
  ...
  if(!html.includes('water-ui2.js')){
    html=html.replace(
      '</body>',
      '<script src="./water-ui2.js?build='+BUILD+'"></script>\n</body>'
    );
  }
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    if(!patched.includes(BUILD)||!patched.includes('progressBar')||!patched.includes('water-ui2.js')||!patched.includes(BACKEND)){
      throw new Error('Bản UI2 chưa hợp lệ');
    }
    ...
    if(!ui.ok)throw new Error('Không tải được water-ui2.js');
    await cache.put(UI,ui.clone());

    const oldQR=await caches.match(QR);
    if(oldQR)await cache.put(QR,oldQR);
    else await cache.add(new Request(QR,{mode:'cors',cache:'reload'}));
...
self.addEventListener('message',event=>{
  ...
  event.waitUntil((async()=>{
    ...
    if(page){
      ...
      ready=ready&&text.includes(BUILD)&&text.includes('water-ui2.js')&&text.includes('progressBar');
    }
  ...
  event.respondWith((async()=>{
    ...
    if(isUI){
      try{
        ...
        if(fresh.ok){
          await cache.put(UI,fresh.clone());
          return fresh;
        }
      }catch(e){}
